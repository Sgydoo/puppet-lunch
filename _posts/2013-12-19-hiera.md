...
Puppet hosts to play with so we'll verify the configuration on these
nodes first. To keep things simple, we'll follow the Hiera
...
You can also find an account of <a
href="{{ site.baseurl }}/puppet/roles-and-profiles.html">how we
implemented Roles and Profiles</a> in Appendix C, but it's best to get to
grips with Hiera and try a few things out yourself before diving into that.
